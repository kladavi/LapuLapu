...
function Test-CorpusTitleValid {
    <#
        V4.0 Sprint 19 + Sprint 23a hardening: returns @{ ok = <bool>; reason = <string> }.
        Rejects titles that are still noise after Normalize-CorpusTitle.
        Rules:
          - non-empty, >= 3 chars
          - not entirely numeric / decimal score (0.05, 70, 0.083)
          - not a bare person name
          - at least 3 meaningful word tokens
          - at least one alphabetic character
          - Sprint 23a: reject meeting-transcript titles
          - Sprint 23a: reject truncated / mid-word fragments
          - Sprint 23a: reject email-subject fragments (Re: / Fwd:)
          - Sprint 23a: reject Copilot reference-tag artifacts (【..】)
    #>
    ...
    # V4.0 Sprint 23a: meeting-transcript titles - "meeting transcript/summary"
    # is the tell-tale Copilot-generated preamble to raw transcript content.
    # Never legitimately a decision or risk title.
    if ($t -match '(?i)meeting\s+transcript(?:/|\s+or\s+)?summary') {
        return @{ ok = $false; reason = 'meeting_title_leak' }
    }
    if ($t -match '(?i)^(?:meeting|transcript|call notes?|call summary)\b') {
        return @{ ok = $false; reason = 'meeting_title_leak' }
    }
    # Email-subject leak: "Re:" / "Fwd:" / "FW:"
    if ($t -match '(?i)^\s*(?:re|fwd|fw)\s*:\s+') {
        return @{ ok = $false; reason = 'email_subject_leak' }
    }
    # Copilot response reference tags 【1-abc】【2-def】
    if ($t -match '\p{Ps}\d+[-][a-f0-9]{4,}\p{Pe}') {
        return @{ ok = $false; reason = 'copilot_reference_artifact' }
    }
    # V4.0 Sprint 23a: truncated / mid-word fragment detection.
    # A title that ends with a very short 2-3 char word or with a comma+word,
    # AND doesn't end at a natural sentence boundary, is a mid-clause fragment.
    if ($t -match ',\s*[A-Za-z]{1,3}\.{0,3}$') {
        return @{ ok = $false; reason = 'truncated_title' }
    }
    if ($t -match '^\s*Convert\s+.+\s+into\s+a\s+tracked\s+.+\s+with\s+explicit\s+entry\s+criteria') {
        # Common Copilot-generated multi-clause sentence promoted as title.
        return @{ ok = $false; reason = 'sentence_promoted_as_title' }
    }
    # Ends with common cut-off suffix patterns
    if ($t -match '(?i)\s(?:and\s+es|and\s+e|to\s+redu|for\s+the|to\s+the|bu|receiv|includin|whic)\s*$') {
        return @{ ok = $false; reason = 'truncated_title' }
    }
    # Ends in a word that is likely truncated (2-3 letter alpha word after space
    # with no trailing punctuation)
    $lastWord = ($t -split '\s+')[-1]
    if ($lastWord -match '^[A-Za-z]{1,3}$' -and $t -notmatch '[.!?]\s*$') {
        # Allow legitimate 3-letter words like "MMM" / "GBO" / "IAM" at end.
        if ($lastWord -notmatch '^[A-Z]{2,3}$') {
            return @{ ok = $false; reason = 'truncated_title' }
        }
    }
    # Long sentence-like title: 15+ words is more likely a promoted sentence
    # than a real title. Combine with lack of terminal punctuation.
    if ($words.Count -ge 15 -and $t -notmatch '[.!?]\s*$') {
        return @{ ok = $false; reason = 'sentence_promoted_as_title' }
    }
...
# --- V4.0 Sprint 22 - Standardized Markdown Frontmatter --------------------
# Every generated markdown artifact carries YAML frontmatter with consistent
# metadata so downstream tooling (Sprint 23 index, Sprint 24 Quartz portal,
# future linters) can discover + classify content without re-parsing bodies.

function Build-StandardFrontmatter {
    <#
        Returns a YAML frontmatter block (with terminating '---\n') ready to
        be prepended to a generated markdown body.
        Type-specific extras are passed via -Extra hashtable.
    #>
    param(
        [string]     $Type,                  # 'dashboard'|'decision-registry'|'risk-register'|'david-inbox'|'execution-insights'|'morning-briefing'|'trends'|'rejected-items'|'weekly-report'
        [string]     $Title,
        [string]     $GeneratedIso,
        [string]     $Generator = 'scripts/generate-current-focus.ps1',
        [string]     $Version   = 'V4.0-sprint23a',
        [hashtable]  $Extra     = @{}
    )
    # V4.0 Sprint 23a: emit clean YAML frontmatter with no inline comments so
    # strict parsers (Quartz, jekyll, hugo, static-analysis tooling) accept it
    # without special-casing.
    $lines = [System.Collections.Generic.List[string]]::new()
    [void]$lines.Add('---')
    [void]$lines.Add(('type: ' + $Type))
    if ($Title) { [void]$lines.Add(('title: "' + ($Title -replace '"','\"') + '"')) }
    [void]$lines.Add(('generator: ' + $Generator))
    [void]$lines.Add(('generated: ' + $GeneratedIso))
    [void]$lines.Add(('version: ' + $Version))
    [void]$lines.Add(('schema: ui/src/lib/matryoshka-item.ts'))
    if ($Extra -and $Extra.Keys.Count -gt 0) {
        foreach ($k in ($Extra.Keys | Sort-Object)) {
            $v = $Extra[$k]
            if ($v -is [array]) {
                [void]$lines.Add(('{0}: [{1}]' -f $k, ($v | ForEach-Object { '"' + ($_ -replace '"','\"') + '"' }) -join ', '))
            } elseif ($v -is [bool] -or $v -is [int] -or $v -is [double]) {
                [void]$lines.Add(('{0}: {1}' -f $k, $v))
            } else {
                [void]$lines.Add(('{0}: "{1}"' -f $k, ([string]$v -replace '"','\"')))
            }
        }
    }
    [void]$lines.Add('---')
...
function Add-MatryoshkaFrontmatter {
    <#
        Prepends standardized frontmatter to a markdown body. If the body
        already starts with `---\n` (existing YAML frontmatter), this is a
        no-op - we don't stack frontmatters.
    #>
    param([string] $Body, [string] $Type, [string] $Title, [string] $GeneratedIso, [hashtable] $Extra = @{})
    if ($null -eq $Body) { $Body = '' }
    # Idempotency: if the body already has frontmatter, leave it alone.
    if ($Body -match '^\s*---\s*(?:\r?\n)') { return $Body }
    $fm = Build-StandardFrontmatter -Type $Type -Title $Title -GeneratedIso $GeneratedIso -Extra $Extra
    return $fm + $Body
}
...
$trendsMd = Add-MatryoshkaFrontmatter -Body $trendsMd -Type 'trends' -Title 'Focus Trends' -GeneratedIso $generatedIso
...
$decRegMd = Add-MatryoshkaFrontmatter -Body $decRegMd -Type 'decision-registry' -Title 'Decision Registry' -GeneratedIso $generatedIso -Extra @{ open_count = @($decisions | Where-Object { $_.status -ne 'closed' }).Count; total_count = @($decisions).Count }
...
$riskRegMd = Add-MatryoshkaFrontmatter -Body $riskRegMd -Type 'risk-register' -Title 'Risk Register' -GeneratedIso $generatedIso -Extra @{ open_count = @($risks | Where-Object { $_.status -ne 'closed' }).Count; total_count = @($risks).Count; high_count = @($risks | Where-Object { $_.status -ne 'closed' -and $_.severity -eq 'High' }).Count }
...
$md = Build-Dashboard $workstreams $finalResults $model $allFiles $latestRecap
$md = Add-MatryoshkaFrontmatter -Body $md -Type 'dashboard' -Title 'Current Focus Dashboard' -GeneratedIso $generatedIso -Extra @{ workstream_count = @($finalResults.Values).Count; p1_count = @($finalResults.Values | Where-Object { $_.category -eq 'P1' }).Count }
[System.IO.File]::WriteAllText($OUT_MD, $md, (New-Object System.Text.UTF8Encoding($false)))
$json = Build-Json $finalResults $meta
[System.IO.File]::WriteAllText($OUT_JSON, $json, (New-Object System.Text.UTF8Encoding($false)))
...
$insightsMd = Add-MatryoshkaFrontmatter -Body $insightsMd -Type 'execution-insights' -Title 'Execution Insights' -GeneratedIso $generatedIso
...
$inboxMd = Add-MatryoshkaFrontmatter -Body $inboxMd -Type 'david-inbox' -Title "David's Priority Inbox" -GeneratedIso $generatedIso -Extra @{ item_count = @($inboxItems).Count }
...
$briefMd = Add-MatryoshkaFrontmatter -Body $briefMd -Type 'morning-briefing' -Title 'Morning Briefing' -GeneratedIso $generatedIso
...
foreach ($kind in @('decision','risk')) {
    ...
    foreach ($entry in $source) {
        ...
        if ($entry.ContainsKey('stale') -and $entry.stale) { continue }
        if ($entry.status -eq 'closed') { continue }
        # V4.0 Sprint 23a: reject items with empty workstream from canonical
        # emit. An item that couldn't be routed to a workstream shows up as
        # `[]` in the weekly report and can't be actioned/owned. Log for
        # visibility.
        $entryWs = if ($entry.workstream) { [string]$entry.workstream } else { '' }
        if ([string]::IsNullOrWhiteSpace($entryWs)) {
            $srcPath = ''
            if ($entry.sourceFiles -and @($entry.sourceFiles).Count -gt 0) { $srcPath = [string]@($entry.sourceFiles)[0] }
            Add-TitleDropRecord -Kind $kind -RawTitle ([string]$entry.title) -NormalizedTitle ([string]$entry.title) -Reason 'empty_workstream' -SourcePath $srcPath
            continue
        }
        $candidateCount++
...
$rejectedMd = Add-MatryoshkaFrontmatter -Body $rejectedMd -Type 'rejected-items' -Title 'Rejected Items (validator v2)' -GeneratedIso $generatedIso -Extra @{ candidate_count = [int]$candidateCount; accepted_count = [int]$acceptedCount; rejected_count = [int]$rejectionRecords.Count }
...
foreach ($scanRoot in $indexScanRoots) {
    if (-not (Test-Path $scanRoot)) { continue }
    $files = @(Get-ChildItem -LiteralPath $scanRoot -Recurse -File -Filter '*.md' -ErrorAction SilentlyContinue)
    foreach ($f in $files) {
        # V4.0 Sprint 23a: use case-insensitive Substring instead of Replace so
        # paths on case-preserving-but-case-insensitive filesystems (Windows /
        # OneDrive) always produce a valid repo-relative path. Loop variable
        # is `$scanRoot` (not `$root`) because PowerShell variables are
        # case-insensitive and would otherwise shadow the top-level `$ROOT`.
        $abs = $f.FullName
